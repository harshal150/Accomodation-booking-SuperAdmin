...
const AllInvoices: FC<{ settlementId?: string }> = ({ settlementId }) => {
...
useEffect(() => {
  ...
  settlementId: ['SETTLEMENT-1', 'SETTLEMENT-2', 'SETTLEMENT-3'][i % 3]
  ...
  const detailedData = Array.from({ length: 15 }, (_, i) => ({
    ...
    settlementId: i % 3 === 0 ? `SETTLEMENT-${Math.floor(i / 3) + 1}` : null, // Assign settlement IDs to some invoices
  }));

  // Filter by settlementId if provided
  const filtered = settlementId
  ? invoiceData.filter((invoice) => invoice.settlementId === settlementId)
  : invoiceData;
